<template>
  ...
  <div class="fixed inset-0 z-20 mx-auto mt-10 flex flex-col w-2/5 h-3/5 bg-white rounded-md">
    <div class="flex justify-between h-12 border-b-2 m-2">
      <div>
        <i
          @click="$emit('toggleProfileEditModal')"
      ...
      <button
        @click="onSaveProfile"
        class="flex items-center text-white text-sm font-bold bg-primary rounded-md px-4 m-1 hover:bg-dark"
      >
        수정
      </button>
    </div>
    ...
    <div class="h-48">
      <div class="bg-gray-300 h-32 relative">
        ...
        <button
          ...
          class="absolute top-12 left-1/2 fas fa-camera text-gray-300 hover:text-black text-lg z-10"
        ></button>
        ...
        <div class="border-4 border-white bg-gray-100 w-28 h-28 rounded-full absolute -bottom-14 left-2">
          ...
          <button
            ...
            class="absolute top-9 left-11 fas fa-camera text-gray-300 hover:text-black text-lg"
          ></button>
    ...
    <!-- PersonalInfo Input -->
    <div class="flex-1 flex flex-col p-2 space-y-1 overflow-auto">
...
export default {
  setup(_props, context) {
    ...
    const onSaveProfile = async () => {
      ...
      context.emit('toggleProfileEditModal')
    }
